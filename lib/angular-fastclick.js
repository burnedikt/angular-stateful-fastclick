...
ngTouch.directive('ngClick', ['$parse',
  function($parse) {
    ...
    return function(scope, element, attr) {
      ...
      var fastclickableElement = FastClick.attach(element[0]);
      // attach a specific class to the clickable element indicating that it is handled via fastclick
      // this needs to be pushed to the end of the call stack, else some browsers will fail adding the class
      window.setTimeout(function() {
        element.addClass('fastclickable');
      });
      ...
      element.on('click', function(event) {
        ...
        if (angular.isDefined(attr.restoreAfter)) {
          ...
          // the window' scope)
          window.setTimeout(fastclickableElement.reset.bind(fastclickableElement), delay);
        }
      });
      ...
      if (angular.isDefined(attr.cancelTap)) {
        scope.$watch(attr.cancelTap, function(newVal) {
          if (newVal) {
            fastclickableElement.reset();
          }
        });
      }
    };
